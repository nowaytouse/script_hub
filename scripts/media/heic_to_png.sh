#!/bin/bash

# ============================================================================
# 📷 HEIC/HEIF to PNG Converter - Lossless with Health Check
# ============================================================================
#
# Batch converts HEIC/HEIF images to lossless PNG format.
#
# Features:
#   ✅ Whitelist: Only processes .heic, .heif files
#   ✅ Complete metadata preservation (EXIF, XMP, ICC Profile)
#   ✅ Health check validation after conversion
#   ✅ System timestamp preservation
#   ✅ In-place conversion mode
#
# Dependencies:
#   - heif-convert (brew install libheif)
#   - exiftool (brew install exiftool)
#   - ffmpeg/ffprobe (brew install ffmpeg) - for health check
#
# Usage:
#   ./heic_to_png.sh /path/to/images
#   ./heic_to_png.sh --in-place /path/to/images
#   ./heic_to_png.sh --skip-health-check /path/to/images
#
# ============================================================================

# Configuration
IN_PLACE=false
TARGET_DIR=""
SKIP_HEALTH_CHECK=false
HEALTH_PASSED=0
HEALTH_FAILED=0

# Colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
CYAN='\033[0;36m'
BLUE='\033[0;34m'
NC='\033[0m'

# Logging
log_info()    { echo -e "${BLUE}ℹ️  [INFO]${NC} $1"; }
log_success() { echo -e "${GREEN}✅ [OK]${NC} $1"; }
log_warn()    { echo -e "${YELLOW}⚠️  [WARN]${NC} $1"; }
log_error()   { echo -e "${RED}❌ [ERROR]${NC} $1"; }
log_health()  { echo -e "${CYAN}🏥 [HEALTH]${NC} $1"; }

# ============================================================================
# 📊 Progress Bar & Time Estimation
# ============================================================================

START_TIME=0
CURRENT_FILE=0
TOTAL_FILES=0

# Display progress bar
show_progress() {
    local current=$1
    local total=$2
    local filename="$3"
    
    local percent=$((current * 100 / total))
    local filled=$((percent / 2))
    local empty=$((50 - filled))
    
    # Progress bar
    printf "\r\033[K"  # Clear line
    printf "📊 Progress: ["
    printf "${GREEN}"
    printf '%0.s█' $(seq 1 $filled)
    printf "${NC}"
    printf '%0.s░' $(seq 1 $empty)
    printf "] ${percent}%% "
    
    # Current/Total
    printf "(${current}/${total}) "
    
    # Time estimation
    if [ $current -gt 0 ]; then
        local elapsed=$(($(date +%s) - START_TIME))
        local avg_time=$((elapsed / current))
        local remaining=$(( (total - current) * avg_time ))
        
        if [ $remaining -gt 60 ]; then
            printf "| ⏱️  ETA: ~$((remaining / 60))m ${remaining % 60}s"
        else
            printf "| ⏱️  ETA: ~${remaining}s"
        fi
    fi
    
    # Current file (truncate if too long)
    if [ -n "$filename" ]; then
        local display_name="$filename"
        if [ ${#display_name} -gt 40 ]; then
            display_name="${display_name:0:37}..."
        fi
        printf "\n   📄 ${display_name}"
    fi
}

# Clear progress bar and move to next line
clear_progress() {
    printf "\r\033[K"
}

# Health check function
check_png_health() {
    local file="$1"
    [ "$SKIP_HEALTH_CHECK" = true ] && return 0
    
    # Check PNG signature
    local sig
    sig=$(xxd -l 8 -p "$file" 2>/dev/null)
    if [ "$sig" != "89504e470d0a1a0a" ]; then
        log_error "Invalid PNG signature: $(basename "$file")"
        return 1
    fi
    
    # Validate with ffprobe
    if command -v ffprobe &> /dev/null; then
        local probe
        probe=$(ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=p=0 "$file" 2>&1)
        if [ $? -ne 0 ] || [ -z "$probe" ]; then
            log_error "Cannot read media structure: $(basename "$file")"
            return 1
        fi
    fi
    
    # Decode test
    if command -v ffmpeg &> /dev/null; then
        if ! ffmpeg -v error -i "$file" -frames:v 1 -f null - 2>/dev/null; then
            log_error "Cannot decode: $(basename "$file")"
            return 1
        fi
    fi
    
    local size
    size=$(stat -f%z "$file" 2>/dev/null || stat -c%s "$file" 2>/dev/null)
    log_health "✅ Passed: $(basename "$file") ($size bytes)"
    ((HEALTH_PASSED++)) || true
    return 0
}

# Parse arguments
for arg in "$@"; do
    case $arg in
        --in-place)
            IN_PLACE=true
            shift
            ;;
        --skip-health-check)
            SKIP_HEALTH_CHECK=true
            shift
            ;;
        -h|--help)
            echo "📷 HEIC/HEIF to PNG Converter"
            echo ""
            echo "Usage: $0 [options] <target_directory>"
            echo ""
            echo "Options:"
            echo "  --in-place           Replace original files after conversion"
            echo "  --skip-health-check  Skip health validation (not recommended)"
            echo "  -h, --help           Show this help"
            exit 0
            ;;
        *)
            TARGET_DIR="$arg"
            ;;
    esac
done

# Check dependencies
if ! command -v heif-convert &> /dev/null; then
    log_error "heif-convert not found. Install: brew install libheif"
    exit 1
fi

if ! command -v exiftool &> /dev/null; then
    log_error "exiftool not found. Install: brew install exiftool"
    exit 1
fi

if [ -z "$TARGET_DIR" ]; then
    log_error "No target directory specified"
    echo "Usage: $0 [--in-place] [--skip-health-check] <target_directory>"
    exit 1
fi

if [ ! -d "$TARGET_DIR" ]; then
    log_error "Directory does not exist: $TARGET_DIR"
    exit 1
fi

# Safety check
if [ "$IN_PLACE" = true ]; then
    REAL_TARGET_DIR=""
    if command -v realpath &> /dev/null; then
        REAL_TARGET_DIR=$(realpath "$TARGET_DIR")
    else
        REAL_TARGET_DIR=$(cd "$TARGET_DIR"; pwd)
    fi

    FORBIDDEN_PATHS=("/" "/etc" "/bin" "/usr" "/System" "/Library" "/Applications")
    for forbidden in "${FORBIDDEN_PATHS[@]}"; do
        if [ "$REAL_TARGET_DIR" = "$forbidden" ]; then
            log_error "🚫 SAFETY: Cannot operate on protected directory: $forbidden"
            exit 1
        fi
    done
fi

echo "╔══════════════════════════════════════════════╗"
echo "║   📷 HEIC/HEIF to PNG Converter              ║"
echo "╚══════════════════════════════════════════════╝"
echo ""
log_info "📁 Target: $TARGET_DIR"
log_info "📋 Whitelist: .heic, .heif → .png"
[ "$IN_PLACE" = true ] && log_warn "🔄 In-place mode: originals will be replaced"
echo ""

# Count total files for progress bar
echo ""
log_info "📊 Counting files for progress tracking..."
local total_count=0

while IFS= read -r -d '' file; do
    ((total_count++)) || true
done < <(find "$TARGET_DIR" -type f \( -iname "*.heic" -o -iname "*.heif" \) -print0 2>/dev/null)

TOTAL_FILES=$total_count
CURRENT_FILE=0
START_TIME=$(date +%s)

log_info "📁 Found: $TOTAL_FILES files"
echo ""

# Main processing
find "$TARGET_DIR" -type f \( -iname "*.heic" -o -iname "*.heif" \) -print0 | while IFS= read -r -d $'\0' heic_file; do
    ((CURRENT_FILE++)) || true
    show_progress $CURRENT_FILE $TOTAL_FILES "$(basename "$heic_file")"
    echo "──────────────────────────────────────────────"
    log_info "📷 Processing: $(basename "$heic_file")"
    
    output_png="${heic_file%.*}.png"

    if [ "$IN_PLACE" = true ]; then
        temp_png="${heic_file}.png.tmp"
        log_info "🔄 Step 1/4: Converting to temp file..."
        heif-convert "$heic_file" "$temp_png" > /dev/null 2>&1
        
        if [ -f "$temp_png" ]; then
            log_info "📋 Step 2/4: Migrating metadata (EXIF, XMP, ICC)..."
            exiftool -tagsfromfile "$heic_file" -all:all -ICC_Profile -overwrite_original "$temp_png" > /dev/null 2>&1
            
            log_info "⏰ Step 3/4: Preserving timestamps..."
            touch -r "$heic_file" "$temp_png"
            mv "$temp_png" "$output_png"
            
            log_info "🏥 Step 4/4: Health validation..."
            if check_png_health "$output_png"; then
                rm "$heic_file"
                log_success "Done: $(basename "$heic_file") → $(basename "$output_png")"
            else
                log_error "Health check failed, keeping original"
                rm -f "$output_png"
                ((HEALTH_FAILED++)) || true
            fi
        else
            log_error "Conversion failed: $(basename "$heic_file")"
            rm -f "$temp_png"
        fi
    else
        if [ -f "$output_png" ]; then
            log_warn "⏭️  Skip: $(basename "$output_png") already exists"
            continue
        fi

        log_info "🔄 Step 1/3: Converting..."
        heif-convert "$heic_file" "$output_png" > /dev/null 2>&1
        
        if [ -f "$output_png" ]; then
            log_info "📋 Step 2/3: Migrating metadata..."
            exiftool -tagsfromfile "$heic_file" -all:all -ICC_Profile -overwrite_original "$output_png" > /dev/null 2>&1
            touch -r "$heic_file" "$output_png"
            
            log_info "🏥 Step 3/3: Health validation..."
            if check_png_health "$output_png"; then
                log_success "Converted: $(basename "$output_png")"
            else
                log_warn "Health check failed, but file created"
                ((HEALTH_FAILED++)) || true
            fi
        else
            log_error "Conversion failed: $(basename "$heic_file")"
        fi
    fi
    fi
    
    clear_progress
done

echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║   📊 Conversion Complete                     ║"
echo "╚══════════════════════════════════════════════╝"

# Health report
if [ "$SKIP_HEALTH_CHECK" = false ]; then
    echo ""
    echo "🏥 Health Report:"
    echo -e "   ✅ Passed:  $HEALTH_PASSED"
    echo -e "   ❌ Failed:  $HEALTH_FAILED"
    total=$((HEALTH_PASSED + HEALTH_FAILED))
    if [ "$total" -gt 0 ]; then
        rate=$((HEALTH_PASSED * 100 / total))
        echo "   📊 Rate:    ${rate}%"
    fi
fi
