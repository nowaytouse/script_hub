...
log_section "Processing AND Rules"

# AND规则拆分文件路径
AND_RULES_DECOMPOSED="$PROJECT_ROOT/ruleset/Sources/and_rules_decomposed.list"
ALL_AND_RULES="$TEMP_DIR/all_and_rules.tmp"
touch "$ALL_AND_RULES"

# 从所有模块中提取AND规则并拆分
extract_and_decompose_and_rules() {
    local module_dir="$1"
    
    find "$module_dir" -name "*.sgmodule" -type f 2>/dev/null | while read -r module; do
        if grep -q "^AND," "$module" 2>/dev/null; then
            log_info "Extracting AND rules from: $(basename "$module")"
            
            # 提取AND规则并拆分为简单域名规则
            grep "^AND," "$module" 2>/dev/null | while read -r and_rule; do
                # 提取DOMAIN
                echo "$and_rule" | grep -oE "DOMAIN,[^,\)]+" | sed 's/DOMAIN,/DOMAIN,/' >> "$ALL_AND_RULES"
                # 提取DOMAIN-SUFFIX
                echo "$and_rule" | grep -oE "DOMAIN-SUFFIX,[^,\)]+" | sed 's/DOMAIN-SUFFIX,/DOMAIN-SUFFIX,/' >> "$ALL_AND_RULES"
            done
            
            # 从模块中移除AND规则
            sed -i '' '/^AND,/d' "$module"
            log_success "Cleaned AND rules from: $(basename "$module")"
        fi
    done
}

# 处理 surge(main) 和 shadowrocket 目录
extract_and_decompose_and_rules "$PROJECT_ROOT/module/surge(main)"
extract_and_decompose_and_rules "$PROJECT_ROOT/module/shadowrocket"

# 去重并更新拆分文件
if [ -s "$ALL_AND_RULES" ]; then
    and_count=$(sort -u "$ALL_AND_RULES" | wc -l | tr -d ' ')
    log_info "Extracted $and_count decomposed rules from AND rules"
    
    # 合并到拆分文件
    {
        echo "# AND Rules Decomposed - 复合规则拆分为简单域名规则"
        echo "# Auto-generated by merge_adblock_modules.sh"
        echo "# Updated: $(date)"
        echo "# ═══════════════════════════════════════════════════════════════"
        echo ""
        # 保留原有手动添加的规则
        if [ -f "$AND_RULES_DECOMPOSED" ]; then
            grep -v "^#" "$AND_RULES_DECOMPOSED" 2>/dev/null | grep -v "^$" | grep -v "^Auto-generated"
        fi
        # 添加新提取的规则
        sort -u "$ALL_AND_RULES"
    } | sort -u > "$TEMP_DIR/and_decomposed_new.tmp"
    
    mv "$TEMP_DIR/and_decomposed_new.tmp" "$AND_RULES_DECOMPOSED"
    log_success "Updated: and_rules_decomposed.list"
fi
