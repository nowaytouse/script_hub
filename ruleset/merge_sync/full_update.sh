...
# ═══════════════════════════════════════════════════════════════
# Step 6b: Sync SKK & BlackMatrix7 upstream rulesets (自有化)
# ═══════════════════════════════════════════════════════════════
STEP=$((STEP + 1))
if [ "$SKIP_ADBLOCK" = false ]; then
    echo -e "${YELLOW}[$STEP/$TOTAL_STEPS] Sync SKK upstream rulesets...${NC}"
    if [ -f "${SCRIPT_DIR}/sync_skk_upstream.sh" ]; then
        if [ "$VERBOSE" = true ]; then
            "${SCRIPT_DIR}/sync_skk_upstream.sh"
        else
            "${SCRIPT_DIR}/sync_skk_upstream.sh" 2>&1 | grep -E "^\[✓\]|^\[✗\]|^\[INFO\]|rules|removed" || true
        fi
        log_success "SKK upstream sync complete"
    else
        log_warning "Skip: sync_skk_upstream.sh not found"
    fi
else
    echo -e "${YELLOW}[$STEP/$TOTAL_STEPS] Skip SKK upstream sync${NC}"
fi
echo ""

# ═══════════════════════════════════════════════════════════════
# Step 6c: Download AdBlock modules + Merge
# ═══════════════════════════════════════════════════════════════
STEP=$((STEP + 1))
if [ "$SKIP_ADBLOCK" = false ]; then
...
if [ "$SKIP_MODULE" = false ]; then
    ...
    if [ -f "${SCRIPT_DIR}/convert_surge_to_shadowrocket.py" ]; then
        log_info "Converting Surge modules → Shadowrocket..."
        if [ "$VERBOSE" = true ]; then
            python3 "${SCRIPT_DIR}/convert_surge_to_shadowrocket.py"
        else
            python3 "${SCRIPT_DIR}/convert_surge_to_shadowrocket.py" 2>&1 | grep -E "^(✅|🚀|🌐|🎉|转换完成)" || true
        fi
        log_success "Shadowrocket modules generated"
    else
        log_warning "Skip: convert_surge_to_shadowrocket.py not found"
    fi
